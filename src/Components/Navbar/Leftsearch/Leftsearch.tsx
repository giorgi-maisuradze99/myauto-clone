...
interface LeftProps {
  ...
  handlepricefrom: (price: string) => void;
  handlepriceto: (price: string) => void;
  selectedMans: Models[];
  vehicletype: MansItem[];
  manmodels: MansAndModels[];
  filteredcats: Category[];
  chosencats: ForCats[];
  filtervehicletype: (type: number) => void;
...
const Leftsearch: React.FC<LeftProps> = ({
  handledeal,
  handlepricefrom,
  handlepriceto,
  selectedMans,
  ...
  chosencats,
  filtervehicletype,
  handlemancheckboxchange,
  handleremovemanfrommodels,
  handlemodelcheckboxchange,
  handlecatscheckboxchange,
  getmodelslist,
  filter,
  handlefilter,
  handleshow,
  ...
  const [gelSelected, setGelSelected] = useState(true);
  ...
  useEffect(() => {
    handlepricefrom(minPrice);
  }, [minPrice]);

  useEffect(() => {
    handlepriceto(maxPrice);
  }, [maxPrice]);
  ...
  return (
    <div className="LeftSearch">
      <div className="vehicle-type-selector">
        <button
          className={
            vehicleType === "car" ? "vehicle-type-active" : "vehicle-type"
          }
          onClick={() => {
            setVehicleType("car");
            filtervehicletype(0);
        ...
        <button
          className={
            vehicleType === "tractor" ? "vehicle-type-active" : "vehicle-type"
          }
          onClick={() => {
        ...
        <button
          className={
            vehicleType === "bike" ? "vehicle-type-active" : "vehicle-type"
          }
          onClick={() => {
            setVehicleType("bike");
            filtervehicletype(2);
      ...
      <div className="vehicle-parameter-selector">
        <DealType handledeal={handledeal} deal={deal} />
        <Manufacturer
      ...
      <div className="price-selector">
        <div className="price-type-div">
          ...
          <div className="price-type-selector">
            <span
              className={gelSelected ? "gel-span-active" : "gel-span"}
              onClick={() => setGelSelected(true)}
            >
              <svg
                className="gel-svg"
                xmlns="http://www.w3.org/2000/svg"
            ...
            <span
              className={gelSelected ? "usd-span" : "usd-span-active"}
              onClick={() => setGelSelected(false)}
            >
              <svg
        ...
        <div className="price-range">
          <input placeholder="დან" value={minPrice} onChange={priceFrom} />
          <span className="dash"></span>
          <input placeholder="მდე" value={maxPrice} onChange={priceTo} />
        </div>
      </div>
      <button className="search-button" onClick={() =>{handleshow(true); handlefilter(true); filter()}}>
        ძებნა
      </button>
    </div>
  );
